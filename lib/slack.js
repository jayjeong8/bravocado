...
async function fetchMessage(client, channelId, messageTs) {
    ...
    console.log('[fetchMessage] conversations.history result:', {
        channelId,
        messageTs,
        hasMessages: !!result.messages,
        messagesLength: result.messages?.length,
        firstMessageTs: result.messages?.[0]?.ts,
    });
    ...
    if (result.messages && result.messages.length > 0) {
        ...
        console.log('[fetchMessage] ts mismatch, trying thread reply:', {
            requestedTs: messageTs,
            foundTs: message.ts,
        });
        ...
        console.log('[fetchMessage] fetchThreadReply result:', {
            found: !!threadReply,
        });
        if (threadReply) {
            return threadReply;
...
/**
 * Permalink를 사용하여 메시지 조회 (스레드 댓글도 정확하게 조회)
 * @param {object} client - Slack client
 * @param {string} channelId - 채널 ID
 * @param {string} messageTs - 메시지 타임스탬프
 * @returns {Promise<object|null>} 메시지 또는 null
 */
async function fetchMessageWithPermalink(client, channelId, messageTs) {
    // 1. getPermalink으로 정확한 thread_ts 얻기
    const permalink = await client.chat.getPermalink({
        channel: channelId,
        message_ts: messageTs,
    });

    // 2. URL에서 thread_ts 추출
    const url = new URL(permalink.permalink);
    const threadTs = url.searchParams.get('thread_ts');

    // 3. 스레드 댓글이면 conversations.replies, 아니면 conversations.history
    if (threadTs) {
        const result = await client.conversations.replies({
            channel: channelId,
            ts: threadTs,
